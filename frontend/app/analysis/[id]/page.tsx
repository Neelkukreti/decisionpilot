...
import {
  CheckCircle, Clock, Loader2, AlertCircle, ExternalLink,
...
const STATUS = {
  ...
  CLARIFY: { accent: '#DC2626', chip: 'bg-red-50    text-red-700    border border-red-100',    label: 'Needs Attention' },
} as const;
...
// ─── Evidence drawer ──────────────────────────────────────────────────────────
function EvidenceDrawer({ item, onClose }: { item: ActionItem; onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-sm bg-white border-l border-slate-200 overflow-y-auto shadow-2xl flex flex-col u-fade-up"
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          {item.verbatim_quote && (
            <div>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2.5">Source quote</p>
...
function AssignPicker({
  ...
  const toggle = (e: React.MouseEvent) => {
    ...
    if (triggerRef.current) {
      ...
      setPos({ top: r.bottom + 4, left: r.left, flipUp: spaceBelow < 200 });
    }
    setOpen(true);
  };
    ...
    <>
      {current ? (
        <button ref={triggerRef} onClick={toggle}
          className="flex items-center gap-1.5 text-xs bg-white border border-slate-200 rounded-full px-2 py-0.5 hover:border-violet-300 transition-colors">
      ...
      {open && (
        <>
          {/* Backdrop — catches all outside clicks without mousedown/click race */}
          <div
          ...
          {/* Dropdown */}
          <div
            className="bg-white border border-slate-200 rounded-xl shadow-2xl py-1 min-w-[160px]"
            style={{
              position: 'fixed',
              top: pos.flipUp ? undefined : pos.top,
              bottom: pos.flipUp ? (window.innerHeight - pos.top + 34) : undefined,
              left: pos.left,
              zIndex: 9999,
            }}
          >
...
// ─── Action card ─────────────────────────────────────────────────────────────
function ActionCard({
  item, onViewEvidence, index, participants,
}: {
  item: ActionItem;
              ...
              <div className="bg-red-50 border border-red-100 rounded-xl p-3.5 space-y-3">
                ...
                {!item.owner && participants.length > 0 && (
                  <div className="border-t border-red-100 pt-2.5 flex items-center gap-2">
                    ...
                    <AssignPicker current={null} participants={participants} onAssign={() => {}} accent="red" />
                  </div>
                )}
              </div>
            )}
...
// ─── Risk card ────────────────────────────────────────────────────────────────
function RiskCard({ risk, participants }: { risk: Risk; participants: Participant[] }) {
        ...
        <div className="min-w-0 flex-1">
          ...
          {risk.owner ? (
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-red-600">
              <AlertTriangle className="w-3 h-3" />
              <span>No owner — assign immediately</span>
            </div>
          )}
        </div>
      </div>
...
export default function AnalysisPage() {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      try {
        const check = await fetch(`/api/meetings/${meetingId}/results`);
        const existing: AnalysisResult = await check.json();
        ...
        <div className="max-w-5xl mx-auto px-5 py-10 space-y-8">
            ...
            <section className="u-fade-up" style={{ animationDelay: `${(autoItems.length + reviewItems.length) * 48 + 60}ms` }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="text-xs text-red-600 bg-red-50 border border-red-100 rounded-full px-2 py-0.5 font-medium">
                  {clarifyItems.length} blocked · click to see how to resolve
                </span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          ...
          {result.health_score !== undefined && (
            <section className="u-fade-up" style={{ animationDelay: sectionDelay(240) }}>
              <h2 className="text-base font-semibold text-slate-900 mb-4">Meeting Health</h2>
              <div className="bg-white rounded-2xl border border-slate-100 p-6"
                ...
                <AnimatedBar value={score} color={hColor} />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                  {result.execution_prediction && (
                    <p className="text-sm text-slate-600 leading-relaxed">{result.execution_prediction}</p>
                  )}
                  {result.health_weaknesses && result.health_weaknesses.length > 0 && (
                    <div className="space-y-2">
                      {result.health_weaknesses.map(w => (
                        <div key={w} className="flex items-start gap-2 text-sm text-slate-500">
